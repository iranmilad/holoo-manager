...
class TechnicalReportingController extends Controller
{
    ...
    public function index()
    {
        ...
        $webhooks= Webhook::whereDate('updated_at', Carbon::today())->where(['user_id'=>$user->id,])->orderByAsc('updated_at')->get() ;

        $invoices= Invoice::whereDate('updated_at', Carbon::today())->where(['user_id'=>$user->id,])->orderByAsc('updated_at')->get();

        $requests = ProductRequest::where(['user_id'=>$user->id,])->orderByAsc('updated_at')->get();
